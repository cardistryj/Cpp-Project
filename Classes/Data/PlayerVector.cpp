		...
		for (auto player2 : players->playervector)
		{
			if (!player1->eated && !player2->eated)
			{
				if (BackGround::isCircleCover(DEFAULTBGSCALE*(player1->getPosition() - player2->getPosition()),
					...
					player2->getContentSize().width / 2 * player2->spritescale*0.9f) && player1->spritescale > player2->spritescale)
				{
...
bool PlayerVector::aiControl(PlayerVector* players)
{
	//以容器的第一个元素作为代表元判断距离
